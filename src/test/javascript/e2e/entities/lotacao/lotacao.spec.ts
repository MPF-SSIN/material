...
describe('Lotacao e2e test', () => {
  ...
  it('should load Lotacaos', async () => {
    ...
    expect(await lotacaoComponentsPage.getTitle()).to.eq('ssinApp.lotacao.home.title');
  ...
  it('should load create Lotacao page', async () => {
    ...
    expect(await lotacaoUpdatePage.getPageTitle()).to.eq('ssinApp.lotacao.home.createOrEditLabel');
  ...
  it('should create and save Lotacaos', async () => {
    ...
    const selectedPropria = lotacaoUpdatePage.getPropriaInput();
    if (await selectedPropria.isSelected()) {
      await lotacaoUpdatePage.getPropriaInput().click();
      expect(await lotacaoUpdatePage.getPropriaInput().isSelected(), 'Expected propria not to be selected').to.be.false;
    } else {
      await lotacaoUpdatePage.getPropriaInput().click();
      expect(await lotacaoUpdatePage.getPropriaInput().isSelected(), 'Expected propria to be selected').to.be.true;
    }
  ...
  it('should delete last Lotacao', async () => {
    ...
    expect(await lotacaoDeleteDialog.getDialogTitle()).to.eq('ssinApp.lotacao.delete.question');
