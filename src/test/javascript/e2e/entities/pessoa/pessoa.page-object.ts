...
export class PessoaUpdatePage {
  ...
  tipoVinculoSelect = element(by.id('field_tipoVinculo'));
  ...
  async setTipoVinculoSelect(tipoVinculo: string): Promise<void> {
    await this.tipoVinculoSelect.sendKeys(tipoVinculo);
  }

  async getTipoVinculoSelect(): Promise<string> {
    return await this.tipoVinculoSelect.element(by.css('option:checked')).getText();
  }

  async tipoVinculoSelectLastOption(): Promise<void> {
    await this.tipoVinculoSelect.all(by.tagName('option')).last().click();
  }
